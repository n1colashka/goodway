...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function showMoreCars() {
    ...
    var carsSliderEl = document.querySelector('.cars__slider'); // Сколько машин должно отображаться
  ...
  function initCarsSlider() {
    ...
    var carsCheckbox = document.querySelector('.cars__type');
    var moreCarsBtn = document.querySelector('.cars__more-btn');
    var carsSliderEl = document.querySelector('.cars__slider');
    carsCheckbox.addEventListener('click', function () {
      carsCheckbox.classList.toggle('checked');
      carsSliderEl.classList.toggle('business'); // Таймаут, чтобы обновить слайдер точно после скрытия элементов, при клике на бизнес-такси
    ...
    if (window.innerWidth <= 1220) {
      ...
      carsSlider.destroy();
      carsCheckbox.addEventListener('click', function () {
        if (moreCarsBtn.classList.contains('hidden')) return;else showMoreCars();
      });
    }
  }
  ...
  function initStartNumbersRun() {
    var Visible = function Visible() {
      target = document.querySelector('.about__box'); // Все позиции элемента
  ...
  function initReviewsSlider() {
    var reviewsSlider = new Swiper('.reviews__slider', {
      ...
      breakpoints: {
        ...
        1250: {
          slidesPerView: 4 // centerMode: false,

        },
        1600: {// spaceBetween: 60,
          // slidesPerView: 4,
        },
        1601: {// spaceBetween: 70,
        }
      },
      navigation: {
